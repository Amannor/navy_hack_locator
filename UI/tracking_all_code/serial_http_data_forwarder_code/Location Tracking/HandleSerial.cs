...
namespace Location_Tracking
{
    class HandleSerial
    {
        ...
        void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            bgWorker.ReportProgress(0, "Started");

            //Read out passed in options
            Hashtable workeroptions;
            workeroptions = (Hashtable)e.Argument;
            bgWorker.ReportProgress(0, "Port: " + (string)workeroptions["port"]);
            bgWorker.ReportProgress(0, "Server URL: " + (string)workeroptions["serverURL"]);
            ...
            bgWorker.ReportProgress(0, "Serial port opened");

            bgWorker.ReportProgress(0, "Waiting for data");
            ...
            do {
                    ...
                    for (int i = 0; i < totalChar; i++)
                    {
                        ...
                        if ((serialBuffer[i].ToString() == "\r") || (serialBuffer[i].ToString() == "\n"))
                        {
                            ...
                            if (reportLine.Length > 0)
                            {
                                ...
                                //Split into parts
                                Match lineMatches = Regex.Match(line, @"^([A-F0-9]{6})\,([A-F0-9]{6})\,(\d{2})$");
                                if (lineMatches.Success)
                                {

                                    int signal = int.Parse(lineMatches.Groups[3].Value);

                                    //Process the tag ping
                                    handleTagPing(lineMatches.Groups[1].Value, lineMatches.Groups[2].Value, signal);

                                }
                                else if (line == "Reader Powered Up")
                                {
                                    bgWorker.ReportProgress(0, line);
                                }
                                else
                                {
                                    bgWorker.ReportProgress(0, "Unrecognied data from reader: " + line);
                                }
                ...
                sendDataBackground((string)workeroptions["serverURL"]);
        ...
        void sendDataBackground(string serverURL)
        {
            ...
            if (postData.Length > 0) {
                ...
                if (serverURL != "")
                {
                    ...
                    try
                    {
                        ...
                        {
                            bgWorker.ReportProgress(0, "Posted data to server, response: " + sb.ToString());
                        }
                        else
                        {
                            bgWorker.ReportProgress(0, "Post Data Error: Received HTTP status code " + response.StatusCode.ToString());
                            bgWorker.ReportProgress(0, "HTTP Data: " + sb.ToString());
                        }

                    }
                    catch (Exception ex)
                    {
                        bgWorker.ReportProgress(0, "Post Data Error: " + ex.Message);
                    }
