...
function readSingleFile() {
	...
	var toPrint = "<div class=\"singleSoldier\"> <a id=\"singleSoldierId\"> ID <\a> &nbsp&nbsp Sector <a id=\"singleSoldierLS\"> Last Seen </a> </div>";
	
	for (var index = 0; index < mydata.length; index++)
	{
		allSoldiers[mydata[index].node_id] = mydata[index].true_location;
		
		var date = new Date(parseInt(mydata[index].time));
		
		var curr = new Date();
		if ((curr.getTime() - (parseInt(mydata[index].time) * 1000)) > 20000)
		{
			document.getElementById("alert").innerHTML = "<img id=\"alertImg\" src=\"location_track_php_code\\resources\\maps\\alert.png\">" + (curr.getTime() - (parseInt(mydata[index].time) * 1000)).toString();
			toPrint = toPrint + "<div class=\"singleSoldierMis\"> <a id=\"singleSoldierId\">" + mydata[index].node_id + "<\a> <a id=\"singleSoldierSec\">" + mydata[index].true_location + "</a> <a id=\"singleSoldierLS\">" + date.getHours()  + ":" + date.getMinutes() + ":" + date.getSeconds() + "</a>";
		}
		else
		{
			toPrint = toPrint + "<div class=\"singleSoldier\"> <a id=\"singleSoldierId\">" + mydata[index].node_id + "<\a> <a id=\"singleSoldierSec\">" + mydata[index].true_location + "</a> <a id=\"singleSoldierLS\">" + date.getHours()  + ":" + date.getMinutes() + ":" + date.getSeconds() + "</a>";
	...
	for (var key in allSoldiers)
	{
		counters[(parseInt(allSoldiers[key]))-1]++;
	}
	
	document.getElementById("counter1").innerHTML = counters[0];
	document.getElementById("counter2").innerHTML = counters[1];
	document.getElementById("counter3").innerHTML = counters[2];
	document.getElementById("counter4").innerHTML = counters[3];
	document.getElementById("counter5").innerHTML = counters[4];
 }
